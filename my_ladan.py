...
from data_and_config.parser import ConfigParser

import numpy as np
from sklearn import metrics
from data_and_config.law_processed.law_processed import get_law_graph
...
with open('data_and_config/data/w2id_thulac.pkl', 'rb') as f:
    word2id_dict = pk.load(f)
    f.close()

emb_path = 'data_and_config/data/cail_thulac.npy'
...
with tf.Graph().as_default():
    ...
    regualrizer = tf.keras.regularizers.L2(0.)
    ...
    law_input, graph_list_1, graph_membership, neigh_index = get_law_graph(law_relation_threshold, word2id_dict, 15, 100)
    ...
    u_aw = tf.get_variable('u_aw', shape=[1, lstm_size * 2], initializer=tf.keras.initializers.glorot_normal)
    u_as = tf.get_variable('u_as', shape=[1, lstm_size * 2], initializer=tf.keras.initializers.glorot_normal)
    ...
    tf.add_to_collection('losses_1', tf.keras.regularizers.L2(0.0001)(loss_1))
    tf.add_to_collection('losses_2', tf.keras.regularizers.L2(0.0001)(loss_2))
    tf.add_to_collection('losses_3', tf.keras.regularizers.L2(0.0001)(loss_3))
    ...
    ############################--------------initialized over---------------########################
    f_train = pk.load(open('data_and_config/legal_basis_data/train_processed_thulac_Legal_basis.pkl', 'rb'))
    f_valid = pk.load(open('data_and_config/legal_basis_data/valid_processed_thulac_Legal_basis.pkl', 'rb'))
    f_test = pk.load(open('data_and_config/legal_basis_data/test_processed_thulac_Legal_basis.pkl', 'rb'))
